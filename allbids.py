...
class Bidding:
    def __init__(self, url, page):
        ...
        self.exp_list = ['询价','施工','维修','维护','运维','改造','改建','中标','塔筒','塔架','基础','法兰','锚栓','压站','主轴','变压器','变流器','主变','箱变','勘察','设计','线路','道路','监理','备件','吊装','可行性','可研','装置','检测','检修','监测','监督','测试','测评','试验','变更','更换','技改','验收','安装','分包','电缆','光缆','材料','箱式','框架','造价','通信','编码','定检','叶片','倒运','消防','开关','主体','集控','诊断','齿轮','柴油','部件','电池','风扇','充电','故障','消缺','外委','水土','电容','稳控','变桨','滑环','打捆','咨询','测风','风速仪','电压','电源','电阻','电梯','模块','网关','数据','驱动','配件','刹车','升降','防尘','评估','档案','监控','偏航','标识','土建','振动','仿真','通讯','液压','雷电','租赁','端子','紫铜','蓄能','加热','控制','继电器','接口','导流','变频','工控','滤芯','螺栓','熔断器','交换机','集电环','联轴器','电气'] # 电机
    ...
    def get_result(self):
        ...
        if self.chkyzm(): #获取验证码、下载、输入并验证
            response = self.session.post(self.loginurl, data=self.logindata, headers=self.headers)
            ...
            for i in range(1, self.page):
                ...
                for item in item_list:
                    ...
                    if self.chktitle(title): #筛选标题去除相关条目
                        if self.key_title1 in title or self.key_title2 in title:
    ...
    def chkyzm(self):
        ...
        yzm = input('请输入验证码：')
        #chkyzmurl = self.url + '/cblcn/member.login/checkyzm?randomID=' + userid + '&yzm=' + yzm + '&t=' + self.stamp # 检测验证码地址
