...
class Bids:
        ...
        if response:    # 获取对象不为空
            ...
            for item in item_list:
                ...
                if self.key_title in title or self.key_title_else in title:
                    ...
                    content = response.html.find(self.sel_content, first=True).text
                    pubdate = response.html.find(self.sel_pubdate, first=True).text
                    if '：' in pubdate:
    ...
    def get_response(self, url):
        try:
            ...
            proxies = {'http': ''}
            r = self.session.get(url, headers=headers, proxies=proxies, timeout=8) # verify = False
            r.encoding = 'utf-8'
            return r
...
class Hrzb(Bids): #华润招标（网页编码）
    ...
    def get_result(self):
        for i in range(1, 4):
            if i == 1:
                self.url = 'http://www.crpsz.com/zbxx/006001/006001001/secondpagejy.html'
            else:
                self.url = 'http://www.crpsz.com/zbxx/006001/006001001/%d.html' % i
            super().get_result()
...
class Zghzb(Bids): #中广核招标（动态加载）
    ...
    def get_result(self):
        ...
        for i in range(1, 3):
            self.url = 'https://ecp.cgnpc.com.cn/CmsNewsController.do?method=recommendBulletinList&index=provincebuyBulletin&channelCode=zgh_zbgg&rp=20&page=%d' % i
            super().get_result()
...
class Hdzb(Bids): #华电招标（证书验证）
    ...
    def get_result(self):
        warnings.filterwarnings("ignore") #忽略InsecureRequestWarning
        for i in range(1, 2):
            self.url = 'https://www.chdtp.com/webs/queryWebZbgg.action?zbggType=1&page.pageSize=31&page.currentpage=%d' % i
            super().get_result()
        print(Fore.RED + '华电招标公告查询完毕！')

    def get_href(self, item):
        return 'https://www.chdtp.com/staticPage/' + item.attrs['href'].split("'")[1]
...
class Hnzb(Bids): #华能招标
    ...
    def get_result(self):
        for i in range(5):
            self.url = 'http://ec.chng.com.cn/ecmall/more.do?type=103&start=%d&limit=10' % (i*10)
            super().get_result()
        print(Fore.CYAN + '华能招标公告查询完毕！')

    def get_href(self, item):
        return 'http://ec.chng.com.cn/ecmall/announcement/announcementDetail.do?announcementId=' + item.attrs['href'].split("'")[1]
...
class Hbzb(Bids): #河北建投招标
    ...
    def get_result(self):
        for i in range(1, 4):
            self.url = 'http://hebeibidding.com/TPFront/xmxx/016001/?Paging=%d' % i
            super().get_result()
...
class Shzb(Bids): #神华招标
    ...
    def get_result(self):
        for i in range(1, 5):
            self.url = 'http://www.shenhuabidding.com.cn/bidweb/001/001002/%d.html' % i
            super().get_result()
...
class Sxzb(Bids): #三峡招标（动态加载，获取json文件）
    ...
    def get_result(self):
        for i in range(2):
            self.url = 'http://epp.ctg.com.cn/index/getData.do?queryName=ctg.list.zbgg&page=%d&rows=15' % i
            response = self.get_response(self.url)
            item_list = response.json()['rows']
            for item in item_list:
                t = time.time()
                href = 'http://epp.ctg.com.cn/static/temphtml/' + item['ARTICLE_ID'] + '.html?_=' + str(round(t * 1000)) #毫秒级时间戳
                title = item['TITLE']
                if self.key_title in title:
                    response = self.get_response(href)
                    content = response.html.find(self.sel_content, first=True).text
...
class Dtzb(Bids): #大唐招标
    ...
    def get_result(self):
        for i in range(1, 5):
            self.url = 'http://www.cdt-ec.com/potal-web/pendingGxnotice/where?message_type=0&pageno=%d&pagesize=15' % i
            response = self.get_response(self.url)
...
class Cebpub(Bids): #招投标公共服务平台
    ...
    def get_result(self):
        ...
        try:
            ...
            searchurl = 'http://bulletin.cebpubservice.com/xxfbcmses/search/bulletin.html'
            for i in range(1, 6):
                data = {
                    'searchDate' : date,
                    'dates' : 3,
                ...
                response = self.session.get(searchurl, headers=headers, params=data, timeout=8)
                item_list = response.html.find(self.sel_title)
                #print(item_list)
                if item_list:
                    for item in item_list:
